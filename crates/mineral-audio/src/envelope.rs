...
//!
//! 峰值统计会在强压限音乐上饱和,平直 RMS 又会让鼓与 sub-bass 的能量高于主观听感。
//! 因此包络采用 ITU-R BS.1770 的 K-weighting(高频搁架 + 低频高通)与 momentary
//! 时间窗。刻意偏离标准 LUFS 的部分:mono 下混后再滤波(与
...
/// 包络算法版本:滤波 / 分块统计 / 归一 / 量化的结构变更时 bump;读取方版本
/// 不符视同缺失、触发重算。当前值标识 K-weighting、momentary 时窗与固定时长块粒度。
///
