...
mod tests {
    ...
    use crate::test_support::{
        app_in_fullscreen, app_with_playlists_probed, app_with_queue, app_with_search,
    };

    /// Playlists 稳态右栏:预热进入后目标曲目封面,但可见封面仍是 playlist 自己的封面。
    #[test]
    fn playlists_prewarms_entry_track_cover_without_replacing_playlist_cover()
    -> color_eyre::Result<()> {
        use std::sync::Arc;
        ...
        let playlist_url = MediaUrl::remote("https://prewarm/playlist.jpg")?;
        let entry_url = MediaUrl::remote("https://prewarm/entry.jpg")?;
        if let Some(playlist) = app.state.library.playlists.get_mut(0) {
            playlist.data.cover_url = Some(playlist_url.clone());
        }
        let mut entry = crate::test_support::song("entry");
        entry.cover_url = Some(entry_url.clone());
        ...
        let mut playlist_img = image::RgbImage::new(64, 64);
        for p in playlist_img.pixels_mut() {
            *p = image::Rgb([255, 0, 255]);
        }
        let mut entry_img = image::RgbImage::new(64, 64);
        for p in entry_img.pixels_mut() {
            *p = image::Rgb([0, 255, 0]);
        }
        app.state.covers.cache.insert(
            playlist_url.clone(),
            Arc::new(image::DynamicImage::ImageRgb8(playlist_img)),
        );
        app.state.covers.cache.insert(
            entry_url.clone(),
            Arc::new(image::DynamicImage::ImageRgb8(entry_img)),
        );

        let mut t = Terminal::new(TestBackend::new(100, 30))?;
        ...
        let warmed = app
            ...
            .iter()
            .any(|(u, _)| *u == entry_url);
        assert!(warmed, "Playlists 稳态应预热入口目标曲目封面");

        let playlist_color = Color::Rgb(255, 0, 255);
        let entry_color = Color::Rgb(0, 255, 0);
        let buf = t.backend().buffer();
        let mut playlist_cells = 0usize;
        let mut entry_cells = 0usize;
        for y in 0..30u16 {
            for x in 0..100u16 {
                if let Some(cell) = buf.cell((x, y)) {
                    if cell.fg == playlist_color || cell.bg == playlist_color {
                        playlist_cells = playlist_cells.saturating_add(1);
                    }
                    if cell.fg == entry_color || cell.bg == entry_color {
                        entry_cells = entry_cells.saturating_add(1);
                    }
                }
            }
        }
        assert!(playlist_cells > 0, "右栏应仍绘制 playlist 自己的封面");
        assert_eq!(entry_cells, 0, "入口目标曲目封面只预热,不替换可见封面");
        Ok(())
    }
    ...
    /// 滚动防抖期(`is_scrolling` 真):封面已解码进 `covers.cache` 但 kitty 协议未编码 →
    /// 应画 halfblock 真图(不再留空),且**不投递 kitty 编码**(压到停稳后,避免给 worker
    /// 灌滚过即弃的图)。于是滚动手感为「低清(滚动期)→ 高清(停稳后)」,无空白阶段。
    /// 构造同 [`fullscreen_steady_pending_encode_shows_halfblock_not_hash`],仅把
    /// `last_sel_change` 设为此刻(落入防抖窗口)。
    #[test]
    fn scroll_debounce_shows_halfblock_and_defers_encode() -> color_eyre::Result<()> {
        use std::sync::Arc;
        ...
        app.state
            ...
            .cache
            .insert(url, Arc::new(image::DynamicImage::ImageRgb8(img)));
        // 落入滚动防抖窗口:is_scrolling() 为真。
        app.state.browse.nav.last_sel_change = Instant::now();

        // 全屏稳态(一步到位);不放已编码协议 → 走「kitty 未编码」分支,撞 is_scrolling 闸。
        let mut fs = Toggle::new(1);
        fs.set(true);
        fs.tick();
        app.state.browse.fullscreen = fs;
        ...
        assert!(
            count > 0,
            "滚动防抖期应画 halfblock 真图(品红)、不再留空;实得 {count}"
        );
        assert!(
            app.state.covers.encode_pending.borrow().is_empty(),
            "滚动防抖期不应投递 kitty 编码(压到停稳后)"
        );
        Ok(())
    }
