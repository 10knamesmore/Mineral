...
/// 渲染一帧:计算布局,填充各面板;退出收缩动画进行时叠加收缩边框。
pub fn draw(frame: &mut Frame<'_>, app: &App) {
    let areas = compute(frame.area());
    paint(frame, &areas, app);
    if let Some(anim) = &app.quit_anim {
        quit_shrink(frame, frame.area(), anim.eased(), &app.theme);
    }
}
...
fn paint(frame: &mut Frame<'_>, areas: &Areas, app: &App) {
    ...
    // 浮层栈:退出收缩动画期间不画(此时收缩的是主界面整体,不带浮层)。
    if app.quit_anim.is_none() {
        app.overlays.render(frame, frame.area(), &app.state, theme);
    }
}

/// 退出收缩:一个整 cell 边框从 `area` 全屏按 `scale`(千分比)收向中心,框内保留已画的
/// 界面内容(静止),框外清成背景色 —— 整个界面像被边框从四周吞没、吸成一点。
fn quit_shrink(frame: &mut Frame<'_>, area: Rect, scale: u16, theme: &Theme) {
