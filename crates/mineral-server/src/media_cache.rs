//! 音频本体的本地缓存:在通用 [`BlobCache`] 之上,补"按歌组织可读库路径"的领域逻辑。
//!
//! 落盘布局 `<source>/<quality>/<album>/<title>.<ext>`(可读、无 hash,可被播放器直接打开);
//! index 键 `media_cache_key` = `{song_id.qualified()}:{quality}`,每音质一份。
...
/// 音频本体缓存。线程安全(内部 [`BlobCache`] 自带锁),不依赖 tokio runtime。
pub(crate) struct MediaCache {
    /// 后端 blob 缓存;`None` = 缓存禁用(null-object)。
    inner: Option<BlobCache>,
...
impl MediaCache {
    ...
    pub(crate) fn open(dir: &Path, capacity: u64) -> color_eyre::Result<Self> {
        let cache = BlobCache::open(dir, capacity)?;
    ...
    pub(crate) fn capture_path(&self, song_id: &SongId, quality: BitRate) -> Option<PathBuf> {
        ...
        let safe = sanitize_segment(&media_cache_key(song_id, quality), "capture");
    ...
    pub(crate) fn get(&self, song_id: &SongId, quality: BitRate) -> Option<PathBuf> {
        self.inner.as_ref()?.get(&media_cache_key(song_id, quality))
    }
    ...
    pub(crate) fn put_played(
        ...
        let key = media_cache_key(&song.id, quality);
        let (subdir, file_name) = library_relpath(song, quality, format);
        cache.put_file(&subdir, &key, src, &file_name)
    }
}
...
/// 缓存 index 键:`{song_id.qualified()}:{quality}`,全局唯一、每音质独立。
///
/// # Params:
///   - `song_id`: 歌曲 ID
///   - `quality`: 音质
...
fn media_cache_key(song_id: &SongId, quality: BitRate) -> String {
...
mod tests {
    ...
    use super::{
        MediaCache, ext_for, media_cache_key, media_relpath, sanitize_segment, truncate_bytes,
    };
    ...
    fn key_includes_qualified_and_quality() {
        ...
        assert_eq!(
            media_cache_key(&s.id, BitRate::Lossless),
            "netease:186016:lossless"
        );
        assert_eq!(
            media_cache_key(&s.id, BitRate::Exhigh),
            "netease:186016:exhigh"
        );
        assert_ne!(
            media_cache_key(&s.id, BitRate::Lossless),
            media_cache_key(&s.id, BitRate::Exhigh),
    ...
    #[test]
    fn put_played_then_get_roundtrips() -> color_eyre::Result<()> {
        let d = tempfile::tempdir()?;
        let cache = MediaCache::open(d.path(), 1_000_000)?;
        let s = song("186016", "晴天", Some("叶惠美"));
        // 模拟 capture 落盘文件。
        let src = d.path().join("capture.part");
        std::fs::write(&src, b"FLACdata")?;
        cache.put_played(&s, BitRate::Lossless, &AudioFormat::Flac, &src)?;
    ...
    #[test]
    fn capture_path_under_tmp() -> color_eyre::Result<()> {
        let d = tempfile::tempdir()?;
        let cache = MediaCache::open(d.path(), 1_000)?;
    ...
    #[test]
    fn disabled_is_null_object() -> color_eyre::Result<()> {
