...
/// Song 在某个 Album canonical snapshot 中的 membership relation。
...
impl AlbumTrack {
    /// 按输入 Song projection 的顺序构造连续的 0-based Album relation。
    ///
    /// 只应在 channel 已形成完整 canonical projection 的 adapter boundary 使用；partial
    /// hydration 若需要保留 gap，应显式携带原 index 构造 relation。
    ///
    /// # Params:
    ///   - `songs`: 已按 canonical album order 排列的 Song
    ///
    /// # Return:
    ///   带连续 0-based index 的 AlbumTrack 列表。
