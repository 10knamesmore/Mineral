...
* **进程级 e2e** → `CARGO_BIN_EXE_<bin>` 起真二进制(见 `crates/mineral/tests/daemon_lifecycle.rs` 的 `Daemon` 框架:隔离 XDG + 子进程 + 读日志),覆盖 daemon 生命周期 / CLI 退出码 / socket 等。

### insta 用法约定

* **所有快照断言必须带中文 `description`**,`cargo insta review` 时逐张显示、便于辨认。统一走 `with_settings!({ description => "…" }, { … })`;TUI 侧用 `mineral-tui` 的 `assert_snap!("…", backend)` 宏(已内置 description)。
...
* **CI**(`.github/workflows/ci.yml`,PR + push main 触发,`main` 的 required check):`cargo fmt --all --check` → `cargo clippy --workspace --all-targets -- -D warnings` → `cargo test --workspace`(`INSTA_UPDATE=no`,快照不匹配/缺基准直接挂)。headless runner 上用 PulseAudio null sink 顶 audio engine,让 daemon e2e 起得来。
* **本地 git hooks**(`.githooks/`,走 `core.hooksPath`):**pre-commit** 只 `cargo fmt --all --check`(秒级);**pre-push** 跑 `clippy -D warnings` + `cargo test`。新 clone 后启用一次:`git config core.hooksPath .githooks`。
