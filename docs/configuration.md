...
14 个颜色 token。每个 token 的色值可以是固定色 `"#rrggbb"`、终端 ANSI 槽(跟随终端配色)或终端默认——写法详见下方[色值写法](#色值写法)。默认主题是 [Catppuccin Mocha](https://catppuccin.com/)(全固定色);想整体换主题,把 14 个 token 全写一遍即可。
...
`background` 是整屏背景填充(全屏沉浸态铺底):默认 `"base"`(主题主背景 token),可写任意 token 名 / `"#rrggbb"` 色值,或 `{ reset = true }` 用终端默认底(不填充)。
...
`text_alpha` 子表是文本层级的不透明度(0-1):歌词渐暗 / transport 标签等次级文本的前景色 = 所在处**实际背景色**与 `text` 按此比例混合——背景是全屏氛围渐变场还是纯底色,层级对比都成比例保持。没铺背景色的面按 `base` 混合(所以不开氛围背景也生效);`base` 为 ANSI / 终端默认时才回落静态 token(括号内)。`strong` / `muted` / `faint` 默认值恰好把 Catppuccin 的中间色阶复现在 `base` 底色上:
...
> `search_hit.color` / 来源徽标色除以上写法外,还能直接写 token 名(如 `"peach"`)引用某个主题 token;14 个 token 本身不能互相引用(它们**被**引用,不去引用别人)。
...
| `open_help` | `?` | 打开快捷键 cheatsheet |
| `jump_to_current` | `c` | 队列浮层:光标跳回在播条目 |
| `reorder_down` / `reorder_up` | `<C-j>` / `<C-k>` | 队列浮层:选中条目下移 / 上移一格 |
...
四种渲染风格由 `style` 切换:`"bars"` 频谱柱(默认)/ `"scope"` 示波器 / `"waterfall"` 频谱历史瀑布 / `"terrain"` 山脊地形。顶层旋钮(DSP 标定 / ADSR / 配色)四风格共用;每种风格的专属参数放各自子表(`spectrum.bars` / `spectrum.scope` / `spectrum.waterfall` / `spectrum.terrain`),仅所选风格的子表生效。

bars 条高单位是 1/8 字符格,满高 64(8 行 × 8)。所有时长旋钮均为毫秒,按 `animation.frame_tick_ms` 折算成拍、与帧率解耦。
...
| `dot_bg_contrast` | 0.25 | 盲文点阵风格(scope / terrain)落笔色与实际背景的最小亮度差(0-1);氛围背景与谱色同源撞色时向亮 / 暗侧抬开,0 = 关闭保底 |
...
| `contrast` | 1.4 | 幅度 → 热力色的对比 gamma:1 = 线性;>1 压暗噪底只留强峰,<1 抬亮弱谐波泛音。渲染层现读即时生效 |
...
包络由 daemon 离线解码整曲算出、落库复用,只对**本地曲库 / 已缓存 / 已下载**的完整媒体可算。包络未就绪时自动回落普通进度条,不占额外空间。首次流式播放期间,producer 下载完整媒体后立即入缓存并计算包络;若当前曲仍在播放,包络到达时直接开始揭示动画。下载完成前切歌不会使用半截文件。
...
`reveal` 子表(入场揭示动画)。包络到达时波形不瞬变:揭示边从左扫到右,每列在被扫到时
从底部长到目标高度,两个方向合成一条对角线。未揭示的列维持普通进度条中线,故整个过程
读作「波形把中线推走」而非凭空出现。**每首歌放一次**——同一首歌内包络重复送达
(play_url 更新等)不重播,seek / 暂停也不重播:

| 字段 | 默认 | 说明 |
|---|---|---|
| `duration_ms` | 560 | 动画总时长;`0` = 一帧到位(等同关闭) |
| `sweep_ratio` | 0.62 | 横扫占总时长比例,余下为单列生长时长;`1` = 纯左→右擦除(列一到位就是满高,无「从下到上」),`0` = 全条同时抬升(无横向次序) |
| `glow` | 0.85 | 揭示边前沿的提亮强度 0-1:正在生长的列朝主题 `text` 混色,越接近到位越回落本色;`0` = 无亮边 |

> 块字符只有 8 级高度,安静段(1~2 级)的「从下到上」至多跳 1 格,生长感主要来自响的列
> ——这是终端的字形颗粒度,调 `duration_ms` 改不了。
...
| 字段 | 默认 | 说明 |
|---|---|---|
| `protocol` | `"auto"` | 终端图协议:`"auto"` 启动探测协商,已知「探测穿透、渲染不合成图数据」的环境(如 zellij)自动降级 halfblock;可指定 `"halfblocks"` / `"kitty"` / `"sixel"` / `"iterm2"`,其中 Kitty 只在 POSIX shared memory 可用时生效 |
| `http_timeout_secs` | 30 | 单张封面下载超时,秒 |
| `debounce_ms` | 32 | 列表滚动停稳多久才解码并编码高清真图;期间优先显示真实 preview，preview 也未就绪才留空 |
...
解码保持原图比例，选择宽高均达到目标的最低可用 JPEG 缩放档位，原图较小时不放大。档位可能高于配置尺寸，不再额外精确缩放。其他格式先完整解码再等比缩小，因此仍有完整解码的瞬时内存开销。窗口或封面区域变化不重新解码；配置热更用于后续解码，已有缓存自然淘汰后更新。

`cache` 子表(缓存预算;LRU,满了自动驱逐,磁盘项改小不立刻删文件):

| 字段 | 默认 | 说明 |
|---|---|---|
| `disk` | `4 * 1024 ^ 3`(4 GiB) | 原始压缩字节的磁盘缓存上限 |
| `image` | `128 * 1024 ^ 2`(128 MiB) | 已准备的高清像素 RAM 预算;越界逐出最久未显示者 |
...
snap 回终端图协议高清。转场窗口恰好盖住新图的离线编码期,落定无空白闪。仅在新旧两图
...
| 字段 | 默认 | 说明 |
|---|---|---|
| `scale` | 1.12 | 缩放幅度:下一首时旧图放大到此倍数退场、新图从此倍数回缩落定;上一首反向,退场图缩小到此倍数的倒数;1 = 无缩放(等效 fade) |
...
| `intensity` | 0.4 | 渐变场浓度 0-1:0 = 纯主题底色;过大前景对比度下降 |
| `sigma` | 0.2 | 锚点高斯半径(屏幕相对):越小色斑越聚拢,越大场越均匀 |
...
| 字段 | 默认 | 说明 |
|---|---|---|
| `strength` | 0.75 | 边缘向底色收敛的强度 0-1;0 = 关闭 |
...
| `sway_pct` | 20.0 | 锚点绕锚位游走半径,屏幕尺寸的 %;太小则场几乎不变形 |
...
`pulse` 子表(响度跳动:正在播放的音频每帧过低通滤波算 RMS 响度,在近期峰值与
谷值之间归一(把压限压扁的动态重新撑开,响度整体偏小 / 压得极扁的歌都有满幅跳动),
过 gamma 感知曲线后由主包络(attack/release)与 punch 瞬态通道合成,按 `depth`
叠加到场浓度上——音乐越响封面色越浓,随鼓点呼吸;暂停 / 静音时经 release 自然回落):

| 字段 | 默认 | 说明 |
|---|---|---|
| `enabled` | `true` | 关闭则场完全不随响度变化 |
| `depth.intensity` | 0.2 | 场浓度增量:越响封面色越浓——最直觉的「颜色跳动」;过大明暗闪烁刺眼 |
| `depth.sigma` | 0.25 | 色斑半径膨胀比例:越响色斑越大(呼吸感);单独开时颜色深浅不变 |
| `depth.brightness` | 0.05 | 亮端推:响度把锚点色带采样位向亮端推,颜色变亮而非变浓 |
| `depth.vignette` | 0.1 | 暗角减弱比例:越响暗角越开、场向边缘涌;易泵感,慎用 |
| `attack_ms` | 70 | 主包络 attack:越小越跟得上鼓点,过小会抖 |
| `release_ms` | 420 | 主包络 release:越大鼓点间衰减越平滑,过小会闪 |
| `gain_window_secs` | 4.0 | 归一跟踪窗口:峰值 / 谷值按此窗口收敛;越大越跟段落起伏 |
| `bass_cutoff_hz` | 150.0 | 低通滤波截止频率:响度只计低频能量,只跟底鼓 / 贝斯走;0 = 不滤波 |
| `gamma` | 2.6 | 感知曲线:归一响度的幂次,>1 鼓点间隙沉得更干净;1 = 线性 |
| `punch.gain` | 0.4 | 瞬态通道混入强度:零 attack 快 release 的第二条包络与主包络取较大者,专抓鼓点的「点」;0 = 关闭 |
| `punch.release_ms` | 160 | punch release:越小「点」越锐利 |
...
| 字段 | 默认 | 说明 |
|---|---|---|
| `radius` | 64 | 列表选中行上下各prefetch数量;同时预取歌单曲目 |
| `playback_cover_radius` | 3 | 沿播放队列给在播曲前后各预取几张封面,服务自动切歌 |
| `prewarm_ahead` | 1 | 在播曲及其前后各几首提前完整解码(切歌转场要两图已解码),并在全屏稳态按此数提前编码,消自动切歌的空白闪 |
...
| `minimap_cursor_ms` | 240 | 列表 minimap 的光标位置缓动与吸附颜色过渡(见 [`tui.minimap`](#tuiminimap--列表位置轨道));0 = 一帧到位 |
...
`ambient_trail` 子表(全屏氛围背景滞后跟随几何形变;进 / 退各一套时长):

| 字段 | 默认 | 说明 |
|---|---|---|
| `enter.delay_ms` / `enter.ease_ms` | 80 / 740 | 进全屏:先僵 `delay` 再 ease-out 缓入;都 0 = 同步无滞后 |
| `exit.delay_ms` / `exit.ease_ms` | 0 / 240 | 退全屏:迅速收(短缓动几乎不在列表上残留) |

`marquee` 子表(溢出标题滚动:选中行 / 播放栏长歌名):

| 字段 | 默认 | 说明 |
|---|---|---|
| `mode` | `"bounce"` | `"loop"` 循环滚动 / `"bounce"` 来回往返 / `"off"` 关闭(静态截断) |
| `step_ms` | 60 | 每前进 1 列的毫秒;越小滚越快(终端无亚像素只能整格跳) |
| `pause_ms` | 100 | 起步 / 选中切换后的停顿毫秒(先读到开头再开滚) |
| `fade_ms` | 300 | 滚动窗口边缘 fade 的渐入毫秒;0 = 关闭边缘 fade |
...
曲目列表 / 歌单列表 / 队列右边框上的整份列表位置轨道:右列盲文轨道,喜欢染红自己那一份等分区间,左列只有光标点(1/4 格精度),在播格显示 `◆`。歌单列表只显示轨道与光标光晕。这里只管几何;光标移动与吸附过渡的时长在 [`tui.animation`](#tuianimation--动画) 的 `minimap_cursor_ms`。

喜欢标记和光晕都遵循 per-item 平分区间：相邻条目的中点作为分界，首末区间贴住轨道两端，单项列表占满轨道。喜欢标记染红条目所属区间；光晕随光标移动，基础半径为条目间距的一半，单项列表覆盖整条轨道，再与最小半径取较大值。范围随过滤后的列表长度和轨道高度变化。强调色从中心向外按 `(1 - 距离 / 半径)^2` 二次衰减，先快后慢淡出，到半径边缘归零。

| 字段 | 默认 | 说明 |
|---|---|---|
| `halo_rows` | 5 | 最小光晕半径(轨道行数):平分范围不足时，向光标上下至少各延伸这么远；0/1 的最小半径均为一行，不关闭光晕 |
| `magnet_dots` | 2 | 吸附区半径(盲文点):光标点离在播点这么近就被吸走(区内不画光标点,在播格淡向光标色、光晕同步淡出);0 = 只有完全重合才算 |
...
| `context` | 否 | 菜单项挂在哪类实体上:`"song"`(默认,收 `mineral.Song`)/ `"playlist"`(收 `mineral.Playlist`,含 `songs`)/ `"album"`(收 `mineral.Album`)/ `"artist"`(收 `mineral.Artist`) |
...
| `fs_spectrum` | `{ pct = 28, min = 10, max = 22 }` | 全屏底部频谱通栏高:占屏高百分比,钳到 `[min, max]` 行(响应式) |
...
把当前播放态实时写进终端任务栏 / tab 标题。三态各一套 `segment` 数组(顺序即输出顺序):`template`(播放 / 暂停共用)、`idle`(无在播)、`disconnected`(未连 daemon)。动态标题(轮换 / spinner / 按宽度自适应)走脚本 `mineral.ui.window_title(text)`,见[脚本指南](./scripting.md)。
...
每个 segment 三种形态:`{ icon = true }`(当前态图标)、`{ text = "..." }`(字面文本)、`{ field = "...", prefix?, suffix?, format? }`(播放态字段,空值整段折叠)。`field` 取值:`title` / `artist` / `album` / `position` / `duration` / `source` / `lyric`。`format`(仅 `position` / `duration`):`"clock"`(默认 mm:ss,≥ 1h 进 h:mm:ss)/ `"seconds"` / `{ pattern = "{m}:{ss}" }`(占位符 `{h}` `{hh}` `{m}` `{mm}` `{s}` `{ss}`)。
...
每个音乐源一张子表。`netease` / `bilibili` 是网络源(超时 / 代理 / 并发 / 徽标色),`mineral` 是聚合收藏源(徽标色 + 后台补全节流)。所有源都有 `color`(来源徽标色,写法同[主题色值](#色值写法))。
...
|---|---|---|
| `color` | `"#2a6511"` | 来源徽标色 |
| `backfill.chunk_size` | 40 | 后台补全 meta:每次 `songs_detail` 调用处理多少 id(= 聚合面刷新粒度) |
| `backfill.max_concurrent` | 3 | 并行几个 `songs_detail` 调用(并发上限即节流) |

各源还可挂 `curate_playlists`(函数字段,对该源歌单列表过滤 / 改名 / 重排);`sources` 表上挂 `curate_playlists` 则对合并后的跨源列表整表变换。详见[脚本指南](./scripting.md)。

## queue — 队列变换

脚本注册的具名队列变换,出现在队列操作菜单的脚本段。`transforms` 数组**整体替换**(默认 `{}`)。每项 `transform` 收有序队列与位置上下文 `ctx = { current = <在播下标>, selected = <光标下标 | nil> }`(均 1-based)、返回新的有序队列;返回的每首歌必须在原队列出现过(按 `id` 认,可复制),混入外来 id 则整次变换被拒。函数在 daemon 脚本运行时执行,详见[脚本指南](./scripting.md)。

| 字段 | 必填 | 说明 |
|---|---|---|
| `label` | 是 | 菜单项显示名 |
| `transform` | 是 | `function(queue, ctx) -> queue`,返回重排后的有序队列 |
| `key` | 否 | 快捷字母;省略 = 仅导航 + 激活可达 |
...
采集侧旋钮只对未来生效;`report` 是查询期口径,改动可回溯重算全部历史。CLI 出报表见 `mineral stats report`。
