...
抓取原始压缩字节 → 磁盘缓存 + 低清 preview → 停稳后按需完整解码 → k-means 取色喂频谱。
...
| `preview` | `8 * 1024 ^ 2`(8 MiB) | 低清 preview RAM 预算;按图片与目标像素尺寸 LRU，完整图 miss 时仍可显示真实封面 |
...
| 字段 | 默认 | 说明 |
|---|---|---|
| `radius` | 64 | 列表选中行上下各预取条数;封面压缩源落磁盘并生成当前布局尺寸的低清 preview,完整解码图不因半径常驻 RAM;同时预取歌单曲目 |
