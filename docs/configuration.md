...
曲目列表 / 队列右边框上的整份列表位置轨道:右列盲文轨道,喜欢染红自己那一份等分区间,左列只有光标点(1/4 格精度),在播格显示 `◆`。这里只管几何;光标移动与吸附过渡的时长在 [`tui.animation`](#tuianimation--动画) 的 `minimap_cursor_ms`。

| 字段 | 默认 | 说明 |
|---|---|---|
| `halo_rows` | 5 | 光晕半径(轨道行数):光标强调色向上下各淡出这么远;0/1 = 只亮光标那一格 |
