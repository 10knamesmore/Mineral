...
抓取原始压缩字节 → 磁盘缓存 → 解码 → k-means 取色喂频谱。
...
| `debounce_ms` | 32 | 列表滚动停稳多久才编码高清真图;期间 halfblock 低清兜底(缓存命中)/ 程序化色块占位(未命中) |
...
| 字段 | 默认 | 说明 |
|---|---|---|
| `radius` | 64 | 列表选中行上下各预取条数;封面只把压缩源预取到磁盘,不保证解码图常驻 RAM;同时预取歌单曲目 |
