...
本仓库的测试细则；`AGENTS.md` 只保留摘要和本页入口。本文记录 Mineral 的测试选型、共享 helper、TUI 集成基建、快照规则与 CI。
...
* **性质 / 不变量(有清晰约束的纯函数)** → **`proptest` 属性测试**。声明"对任意输入恒成立"的性质,框架随机生成数百用例 + 失败自动收缩到最小反例。甜区:codec 编解码往返(`crates/mineral-protocol/tests/codec.rs` 的 `arb_request` round-trip)、解析器抗崩 + 自洽(`mineral-model` LRC:不 panic / 严出幂等)、`color` lerp 范围 / `layout::compute` 子区域不越界。失败种子存 `proptest-regressions/*.txt`,**进 git** 当永久回归用例；`color::lerp_byte` 的回归种子是现有示例。**不适合**渲染 / IO / "输出就该长这样"——那些走 insta。
...
* **进程级 e2e** → `CARGO_BIN_EXE_<bin>` 起真二进制(见 `crates/mineral/tests/daemon_lifecycle.rs` 的 `Daemon` 框架:隔离 XDG + 子进程 + 读日志),覆盖 daemon 生命周期 / CLI 退出码 / socket 等。daemon 可同时服务多个 client；就绪探测与真实请求使用独立连接,不存在独占 client slot 的 busy 竞态。
...
* `song(id)` + 函数式装饰 `with_artist` / `with_name` / `with_source` / `with_duration`(造 `Song`,不要在各 crate 重复定义)。
...
* **动态内容**(时间戳、UUID、临时路径等)用 `with_settings!({ filters => vec![(正则, "占位符")] }, …)` 归一化,否则每次变动都假失败；`insta` 的 `filters` feature 由根 `Cargo.toml` 的 workspace dependency 声明。版本号是例外:`crates/mineral-tui/src/components/layout/shared/top_status.rs` 的 `DISPLAY_VERSION` 在测试构建中固定成占位值,**渲染源头**就不带真实版本,故含顶栏的快照走朴素 `assert_snap!` 即可、无需 per-test filter(避免新增顶栏快照时漏写 filter 又把版本号烤进去)。
...
* HashMap 顺序不稳定用 `with_settings!({ sort_maps => true }, …)`。ratatui `Table` 的列约束若允许多个解,快照仍可能随列宽求解漂移；生产布局必须唯一分配剩余宽度,例如 `components::layout::browse::sidebar::playlists::render_to` 的 `Constraint::Fill`。
